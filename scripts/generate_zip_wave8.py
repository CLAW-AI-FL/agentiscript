...
verticals = [
    'general-surgeon','orthopedic-surgeon','ophthalmologist',
    'urgent-care','walk-in-clinic','physical-rehabilitation',
    'addiction-treatment','fertility-clinic','weight-loss-clinic',
    'concierge-medicine'
]
        ...
        if slug not in existing:
            ...
            if added % 200000 == 0:
                ...
                with open('/Users/colin/seo/agentiscript/agentiscript-index.json','w') as f:
                    json.dump({'total':total_now,'chunks':len(chunks),'chunk_size':chunk_size,'version':'8.0.0'}, f, separators=(',',':'))
                with open(CATALOG) as f:
                    main = json.load(f)
                main['total'] = total_now
                with open(CATALOG,'w') as f:
                    json.dump(main, f, separators=(',',':'))
                os.system(f'cd /Users/colin/seo/agentiscript && git add agentiscript.json agentiscript-index.json agentiscript-chunk-*.json && git commit -m "ZIP8 batch {batch}: {total_now}" && git push 2>&1 | tail -1')
...
with open('/Users/colin/seo/agentiscript/agentiscript-index.json','w') as f:
    json.dump({'total':total_final,'chunks':len(chunks),'chunk_size':chunk_size,'version':'8.0.0'}, f, separators=(',',':'))
with open(CATALOG) as f:
    main = json.load(f)
main['total'] = total_final
with open(CATALOG,'w') as f:
    json.dump(main, f, separators=(',',':'))
os.system(f'cd /Users/colin/seo/agentiscript && git add agentiscript.json agentiscript-index.json agentiscript-chunk-*.json && git commit -m "ZIP8 COMPLETE: {total_final}" && git push')
